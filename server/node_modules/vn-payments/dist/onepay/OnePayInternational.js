 ...
 * this.response.end();
 */
class OnePayInternational extends _OnePay.OnePay {
	/**
  *
  ...
  */
	static getReturnUrlStatus(responseCode, locale = 'vn') {
		const responseCodeTable = {
			...
			2: {
				vn: 'Ngân hàng phát hành từ chối cấp phép, do số dư không đủ thanh toán hoặc chưa đăng ký dịch vụ thanh toán trực tuyến',
  ...
  */
	get checkoutPayloadDefaults() {
		...
		return {
			...
			billingStreet: '',
			currency: _OnePay.OnePay.CURRENCY_VND,
			deliveryAddress: '',
			deliveryCity: '',
			deliveryCountry: '',
			...
			customerPhone: '',
			deliveryProvince: '',
			locale: _OnePay.OnePay.LOCALE_VN,
			title: 'VPC 3-Party',
			customerId: '',
			vpcAccessCode: '',
			vpcCommand: _OnePay.OnePay.COMMAND,
			vpcMerchant: '',
			vpcVersion: _OnePay.OnePay.VERSION
		};
	}
	/**
 ...
 * @author Thanh
 */
OnePayInternational.checkoutSchema = new _simplSchema2.default({
	againLink: { type: String, max: 64, regEx: _utils.urlRegExp },
	...
	customerEmail: { type: String, optional: true, max: 24, regEx: _simplSchema2.default.RegEx.Email },
	...
	returnUrl: { type: String, max: 255, regEx: _utils.urlRegExp }, // NOTE: returnURL is documented with 64 chars limit but seem not a hard limit, and 64 is too few in some scenario
...
exports.OnePayInternational = OnePayInternational;
//# sourceMappingURL=OnePayInternational.js.map